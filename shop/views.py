...
def tracker(request):
    if request.method == "POST":
        ...
        email = request.POST.get('email', '')
        try:
            order = Place_Orders.objects.filter(order_id=orderId, email=email)
            if len(order) > 0:
...
def checkout(request):
    if request.method == "POST":
        ...
        phone = request.POST.get('phone', '')
        order = Place_Orders(items_json=items_json, name=name, email=email, address=address, city=city,
